...
void twindow_setting::pre_base(twindow& window)
{
	...
	toggle->set_did_state_changed(boost::bind(&twindow_setting::automatic_placement_toggled, this, _1));
	...
	toggle->set_did_state_changed(boost::bind(&twindow_setting::did_theme_toggled, this, _1));
...
void twindow_setting::append_float_widget(twindow& window)
{
	...
	gui2::tedit_box_param param(_("New float widget"), null_str, _("ID"), default_float_widget_id, _("OK"));
	{
		param.did_text_changed = boost::bind(&mkwin_controller::verify_new_float_widget, &controller_, _1, nposm);
...
void twindow_setting::rename_float_widget(twindow& window)
{
	...
	gui2::tedit_box_param param(_("Rename float widget"), ss.str(), _("ID"), item.u->cell().id, _("OK"));
	{
		param.did_text_changed = boost::bind(&mkwin_controller::verify_new_float_widget, &controller_, _1, float_widget_at_);
		gui2::tedit_box dlg(param);
		try {
			dlg.show();
		} catch(twml_exception& e) {
