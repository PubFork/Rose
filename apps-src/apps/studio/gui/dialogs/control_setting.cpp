...
void tcontrol_setting::pre_size()
{
	...
	if (u_.has_size()) {
		...
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_best_size_changed, this, _1, true));
		...
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_best_size_changed, this, _1, false));
		...
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_min_text_width_changed, this, _1));
...
void tcontrol_setting::pre_advanced(twindow& window)
{
	...
	if (!u_.is_text_box()) {
		...
		toggle = find_widget<ttoggle_button>(advanced_page, "text_box_password", false, true);
	...
	if (!u_.is_report()) {
		...
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_report_fixed_cols_changed, this, _1));
		...
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_report_unit_size_changed, this, _1, true));
		text_box->set_label(cell_.widget.report.unit_width);

		text_box = find_widget<ttext_box>(advanced_page, "unit_height", false, true);
		text_box->set_placeholder(_("Usable variable: screen_width, screen_height"));
		text_box->set_did_text_changed(boost::bind(&tcontrol_setting::did_report_unit_size_changed, this, _1, false));
...
void tcontrol_setting::click_report_definition(tgrid& grid)
{
	...
	for (std::map<std::string, gui2::tcontrol_definition_ptr>::const_iterator it = definitions.begin(); it != definitions.end(); ++ it) {
		ss.str("");
		ss << ht::generate_format(it->first, color_to_uint32(font::BLUE_COLOR)) << "(" << it->second->description << ")";
		items.push_back(ss.str());
		values.push_back(index ++);
