...
void tanim3::pre_show()
{
	...
	list->set_did_row_changed(boost::bind(&tanim3::did_animation_changed, this, _2));
	...
	report->set_did_item_changed(boost::bind(&tanim3::did_app_changed, this, _2));
	if (!anim_apps_.empty()) {
		report->select_item(0);
	}

	ttree* editor = find_widget<ttree>(window_, "editor", false, true);
	editor->set_did_double_click(boost::bind(&tanim3::did_double_click_editor, this, _2));
	editor->set_did_right_button_up(boost::bind(&tanim3::did_right_button_up_editor, this, _2, _3));
...
std::string tanim3::anims_cfg(const std::string& app) const
{
	...
	if (app.empty()) {
		...
		ss << game_config::path << "/data/app-" << app << "/units-internal/animation.cfg";
	}
...
void tanim3::insert_animation(twindow& window)
{
	...
	frame_cfg["image"] = "misc/arrow-right.png";
...
{
	tanim2& anim = anims_updating_[clicked_anim_];
	gui2::tedit_box_param param(_("Rename id"), null_str, _("Input new id"), anim.id_, _("OK"));
	{
		param.did_text_changed = boost::bind(&tanim3::did_anim_id_changed, this, _1);
