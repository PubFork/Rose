	...
	, gap_argb_(0xffd4d4d4) // 0xff344768, 0xffe8e8ec
{
	...
	files.push_back("c:/ddksample/android-logo-mask.png");
	files.push_back("c:/ddksample/readme.md");
	files.push_back("C:\\import");
	// minizip::zip_file("c:/ddksample/file.zip", files, null_str);
	// minizip::unzip_file("c:/ddksample/file.zip", null_str, null_str, null_str);
}
...
void trose::pre_show()
{
	...
	report->set_did_item_click(boost::bind(&trose::did_toolbar_item_click, this, _2));
...
void trose::did_toolbar_item_click(tbutton& widget)
{
...
bool trose::export_files_must_exist(const tapp_copier& copier) const
{
	struct tcheck_file {
	...
	exist_files.push_back(tcheck_file("assets/Icon-152.png", 152, 152, false, _("Must exist Icon-152.png in images/assets")));
	...
	exist_files.push_back(tcheck_file("assets/Default-414w-736h@3x.png", 1242, 2208, false, _("Must exist Default-414w-736h@3x.png in images/assets")));
...
bool trose::zip_kos_res(const tapp_copier& copier, bool silent)
{
	std::stringstream ss;
	utils::string_map symbols;
...
bool trose::walk_kernels(const std::string& dir, const SDL_dirent2* dirent, std::vector<tcpp_file>& files)
{
	bool isdir = SDL_DIRENT_DIR(dirent->mode);
	bool hit = !isdir;
	if (hit) {
...
bool trose::walk_ops(const std::string& dir, const SDL_dirent2* dirent, std::vector<tcpp_file>& files)
{
	bool isdir = SDL_DIRENT_DIR(dirent->mode);
	bool hit = !isdir;
	if (hit) {
...
void trose::right_click_explorer(ttree_node& node, const tpoint& coordinate)
{
	...
	enum {new_dialog, new_scene, theme, export_app_package, edit_capabilities, export_kos_ksa, copy_res_2_apk, remove_app};
	const int app_wight = 100;
	int at = 0, app_index = 1;
	const int new_app_at = at ++;
	items.push_back(gui2::tmenu::titem(std::string(_("New app")) + "...", new_app_at, "misc/new.png"));
	const int import_app_at = at ++;
	items.push_back(gui2::tmenu::titem(std::string(_("Import app")) + "...", import_app_at, "misc/import.png"));
	...
	for (std::vector<std::unique_ptr<tapp_copier> >::const_iterator it = app_copiers.begin(); it != app_copiers.end(); ++ it) {
		...
		if (apps_sln::can_new_dialog(app.app)) {
			sub_items.push_back(gui2::tmenu::titem(vgettext2("New dialog", symbols) + "...", app_index * app_wight + new_dialog));
			sub_items.push_back(gui2::tmenu::titem(vgettext2("New scene", symbols) + "...", app_index * app_wight + new_scene));
			sub_items.push_back(gui2::tmenu::titem(vgettext2("Theme", symbols) + "...", app_index * app_wight + theme));
		}
		sub_items.push_back(gui2::tmenu::titem(vgettext2("Export app package", symbols) + "...", app_index * app_wight + export_app_package, "misc/export.png"));
		sub_items.push_back(gui2::tmenu::titem(vgettext2("Edit capabilities", symbols) + "...", app_index * app_wight + edit_capabilities));
		sub_items.push_back(gui2::tmenu::titem(vgettext2("Export kOS ksa", symbols) + "...", app_index * app_wight + export_kos_ksa, "misc/export.png"));
		// sub_items.push_back(gui2::tmenu::titem(vgettext2("Copy resource to android's apk", symbols) + "...", app_index * app_wight + copy_res_2_apk));
		if (!is_reserve_app(app.app)) {
			sub_items.push_back(gui2::tmenu::titem(vgettext2("Remove", symbols) + "...", app_index * app_wight + remove_app, "misc/remove.png"));
		}
	...
	const int header_index = app_index;
	...
	if (selected == new_app_at) {
		...
		if (selected % app_wight == new_dialog) {
			// new dialog
			std::string id;
			{
				gui2::tnew_window dlg(false, current_app);
				dlg.show();
				if (dlg.get_retval() != gui2::twindow::OK) {
					return;
				}
		...
		} else if (selected % app_wight == new_scene) {
			// new scene
			std::string id;
			bool unit_files;
			{
				gui2::tnew_window dlg(true, current_app);
				dlg.show();
				if (dlg.get_retval() != gui2::twindow::OK) {
					return;
				}
		...
		} else if (selected % app_wight == theme) {
			// theme
			{
		...
		} else if (selected % app_wight == export_app_package) {
			// export
			export_app(current_app);

		} else if (selected % app_wight == edit_capabilities) {
			// edit capabilities
		...
		} else if (selected % app_wight == export_kos_ksa) {
...
*/
		...
		} else if (selected % app_wight == copy_res_2_apk) {
			// copy resource to android's apk
			symbols["app"] = current_app.app;
			ss.str("");
...
void trose::click_refresh(twindow& window)
{
/*
	{
		const int argc = 7;
...
/*
	{
		net::fetch_url_data("http://www.github.com", game_config::preferences_dir + "/1.html");
		int ii = 0;
	}
*/
/*
	// search_file("c:/data/boot2.img");
	search_file("C:/movie/NanoPi2/s5p4418-eflasher-20180323-android.img");

	extract_file("C:/movie/NanoPi2/s5p4418-eflasher-20180323-android.img", "c:/data/boot3.img", 765702144 + 1136, 1024 * 1024 * 5);

	// mkbootimg("c:/data/zImage", "c:/data/ramdisk.img", "c:/data/boot2.img");

	return;
*/
/*
	{
		treport* report = find_widget<treport>(&window, "toolbar", false, true);
		report->set_item_visible(0, false);	
	}
*/
	fill_items(window);
}
...
void trose::generate_gui_app_main_cfg(const std::string& res_path, const std::set<std::string>& apps) const
{
	...
	for (std::set<std::string>::const_iterator it = apps.begin(); it != apps.end(); ++ it) {
		...
		fp_ss << "{gui/" << game_config::generate_app_dir(app) << "/tpl_widget/}\n";
...
bool trose::collect_app2(const std::string& dir, const SDL_dirent2* dirent, std::vector<std::unique_ptr<tapp_copier> >& app_copiers)
{
	...
	if (isdir) {
		const std::string app = game_config::extract_app_from_app_dir(dirent->name);
		if (!app.empty()) {
			std::string file = dir + "/data/" + dirent->name + "/capabilities.cfg";
