...
std::string app_version_from_settings_cfg(const std::string& app)
{
	...
	config cfg;
	cache.get_config(game_config::path + "/data/" + game_config::generate_app_dir(app) + "/settings.cfg", cfg);
	const config& app_settings_cfg = cfg.child("settings");
	if (!app_settings_cfg) {
		return game_config::rose_version;
	}
...
static bool generate_ios_prj(const tapp_copier& copier, const std::string& app_res_path, const std::string& app_ios_prj_path)
{
	...
	// if (app_in_prj != copier.app) {
	{
		...
		const std::string dst_launchimage = dst + "/Images.xcassets/LaunchImage.launchimage";
		// form Default-414w-736h@3x.png to misc launch image
		full_file = game_config::path + "/" + game_config::generate_app_dir(copier.app) + "/images/assets/";
		file_ss.str("");
		file_ss << full_file << "Default-414w-736h@3x.png";
		surf = image::get_image(file_ss.str());
		bool portrait = surf->w < surf->h;
		if (!portrait) {
			// now roate to portrait always.
			surf = rotate_landscape_anticolockwise90(surf);
			portrait = true;
		}
		std::map<tpoint, std::string> launch_images;
		launch_images.insert(std::make_pair(tpoint(640, 1136), "Default-568h@2x.png"));
		launch_images.insert(std::make_pair(tpoint(750, 1334), "Default-375w-667h@2x.png"));
		launch_images.insert(std::make_pair(tpoint(1242, 2208), "Default-414w-736h@3x.png"));
		for (std::map<tpoint, std::string>::const_iterator it = launch_images.begin(); it != launch_images.end(); ++ it) {
			const std::string dst = dst_launchimage + "/" + it->second;
			// VALIDATE(SDL_IsFile(dst.c_str()), null_str);
			file_ss.str("");
			file_ss << full_file << it->second;
			surface target_surf = image::get_image(file_ss.str());
			int w = portrait? it->first.x: it->first.y;
			int h = portrait? it->first.y: it->first.x;
			if (target_surf && target_surf->w == w && target_surf->h) {
				SDL_CopyFiles(file_ss.str().c_str(), dst.c_str());
			} else {
				surface scaled = scale_surface(surf, w, h);
				IMG_SavePNG(scaled, dst.c_str());
			}
		}
	}
...
bool texporter::collect_cpp_files(const std::string& dir, const SDL_dirent2* dirent, std::map<std::string, uint32_t>& files) const
{
	...
	if (isdir) {
		uint32_t flags = 0;
		walk_dir(dir + "/" + dirent->name, false, boost::bind(&texporter::collect_cpp_files2, this, _1, _2, boost::ref(flags)));
		if (flags) {
...
bool texporter::generate_android_prj() const
{
	std::stringstream ss;
	const std::string app_in_prj = "studio";
	...
	if (app_in_prj != copier_.app) {
		...
		SDL_RenameFile(dst.c_str(), android_mk_tmp.c_str());
		{
			...
			walk_dir(app_dir + copier_.app, true, boost::bind(
					&texporter::collect_cpp_files
					, this
					, _1, _2, boost::ref(cpp_files)));
			ss.str("");
	...
	// if (app_in_prj != copier_.app) {
	{
		...
		for (std::map<int, std::string>::const_iterator it = dpis.begin(); it != dpis.end(); ++ it) {
			const std::string dst = app_android_prj_path + "/app/src/main/res/" + it->second + "/ic_launcher.png";
			VALIDATE(SDL_IsFile(dst.c_str()), null_str);
			file_ss.str("");
			file_ss << full_file << it->first << ".png";
			surface target_surf = image::get_image(file_ss.str());
...
bool texporter::generate_kos_prj() const
{
	...
	// <kos_prj>/app/jni/Android.mk
	if (app_in_prj != copier_.app) {
		const std::string android_mk_tmp = "Android.mk.tmp";
		src = app_kos_prj_path + "/app/jni/" + android_mk_tmp;
		dst = app_kos_prj_path + "/app/jni/Android.mk";
		SDL_RenameFile(dst.c_str(), android_mk_tmp.c_str());
		{
			std::vector<std::string> vstr;
			tfile file(src, GENERIC_WRITE, OPEN_EXISTING);
	...
	// <app_src>/scripts/kos_set_variable.tpl
	{
		...
		// app directory
		std::stringstream app_dir;
		app_dir << "\n\nset " << copier_.app << "=" << app_src2_path << "/projectfiles/kos/app";
