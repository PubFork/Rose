...
int32_t VideoCapturekOS::StartCapture(const VideoCaptureCapability& capability)
{
	...
	if (!setup() || camera2_startPreview(at_, onByteBufferFrameCaptured_static, this) != 0) {
		release();
...
bool VideoCapturekOS::setup()
{
	...
	int ret = camera2_setup(at_, clientPackageName);
	if (ret == 0) {
		initialized_ = true;
	}
