...
int RTSPClient::openConnection1() {
  do {
    ...
    // We don't yet have a TCP socket (or we used to have one, but it got closed).  Set it up now.
	ctrl_socket_ = socket_factory_->CreateTransportClientSocket(addresses_, nullptr, net_log2_.net_log(), net_log2_.source());
    ...
    // Connect to the remote endpoint:
    fServerAddress = *(netAddressBits*)(destAddress.data());
	int connectResult = ctrl_socket_->Connect(envir().iocomplete);
...
int RTSPClient::DoResolveHost() {
  if (host_resolver_.get() == nullptr) {
    ...
    host_resolver_.reset(new net::HostResolverImpl(options, &net_log_));
  }
  next_state_ = STATE_RESOLVE_HOST_COMPLETE;
  
  const GURL gurl(fBaseURL);
  net::HostResolver::RequestInfo info(net::HostPortPair::FromURL(gurl));
  ...
  // No known referrer.
  int rv = host_resolver_->Resolve(info, net::DEFAULT_PRIORITY, &addresses_,
      base::Bind(envir().iocomplete), &resolve_request_, net_log2_);
