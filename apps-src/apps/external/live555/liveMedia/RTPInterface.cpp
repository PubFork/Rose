				 ...
				 Boolean& packetReadWasIncomplete)
{
	...
	if (envir().read_buf->size() < (int)curBytesToRead) {
		envir().read_buf = new net::IOBufferWithSize(posix_align_ceil(curBytesToRead, 4096));
	}
	int times = 0;
	...
	if (fNextTCPReadSize == 0) {
		...
		if (fls_verbos) {
			SDL_Log("handleRead_chromium, set in_io_pending bo true");
		}
		// We need to read more bytes, and there was not an error reading the socket
		packetReadWasIncomplete = True;
		return True;
...
void SocketDescriptor::OnIOComplete(int result)
{
	...
	if (result == SPECIAL_CALL_MAGIC) {
		...
		SDL_Log("SocketDescriptor::OnIOComplete, E, result: %i", result);
...
Boolean SocketDescriptor::tcpReadHandler1_chromium(int rv, bool first_result, bool iocomplete)
{
	...
	if (fTCPReadingState != AWAITING_PACKET_DATA) {
		...
		if (result == 0) { // There was no more data to read
			...
			if (fls_verbos) {
				SDL_Log("tcpReadHandler1_chromium[1], set in_io_pending is true. rv: %i, %s, %s", rv, first_result? "first": "second", iocomplete? "iocomplete": "appcall");
			}
	...
	case AWAITING_PACKET_DATA: {
		...
		if (rtpInterface != NULL) {
			...
			if (rtpInterface->fReadHandlerProc != NULL) {
				...
				if (rtpInterface->fNextTCPReadSize == 0) {
					if (fEnv.setup_finished) {
						RTPSource* source = static_cast<RTPSource*>(rtpInterface->fOwner);
						if (source->isCurrentlyAwaitingData()) {
						if (fls_verbos) {
							SDL_Log("#%i, tcpReadHandler1_chromium, AWAITING_PACKET_DATA, fNextTCPReadSize == 0 && isCurrentlyAwaitingData()", dbg_times);
						}
						callAgain = true;
						break;
						}
