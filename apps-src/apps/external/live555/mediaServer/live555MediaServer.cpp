...
void tmanager::start_internal()
{
	live555d_thread_checker.reset(new rtc::ThreadChecker);

	std::unique_ptr<tauto_destruct_executor> destruct_executor(new tauto_destruct_executor(boost::bind(&tmanager::did_set_event, this)));
...
{
	tauto_destruct_executor destruct_executor(boost::bind(&tmanager::did_set_event, this));
...
void tmanager::start()
{
	fls_verbos = game_config::server_matched;
	...
	thread->task_runner()->PostTask(FROM_HERE, base::BindOnce(&tmanager::start_internal, base::Unretained(this)));
