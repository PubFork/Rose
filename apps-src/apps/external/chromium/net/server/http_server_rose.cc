...
void thttpd_manager::start_internal()
{
	tauto_destruct_executor destruct_executor(boost::bind(&thttpd_manager::did_set_event, this));
...
void thttpd_manager::start()
{
	...
	thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&thttpd_manager::start_internal, base::Unretained(this)));
