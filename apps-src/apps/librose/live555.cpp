...
static std::unique_ptr<base::RunLoop> runloop;
...
*/
    ...
    if (now - last_ms >= 1000) {
		...
		SDL_Log("[live555.cpp]#%u, %u fps[per second]", elapsed_second, received_frames - last_received_frames);
...
int start(const std::string& rtsp_url, std::string& httpd_url)
{
	VALIDATE_IN_MAIN_THREAD();
	instance->will_longblock();

	SDL_Log("%u, live555::start", SDL_GetTicks());
	...
	if (use_chromium) {
		runloop.reset(new base::RunLoop());
		const int timeout = 4500;
		base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::Bind(&rtsp_setup_slice, nullptr, runloop.get(), timeout));
		runloop->Run();
		runloop.reset();
	} else {
	...
	threading::lock lock(live555_ptr->mutex);
...
int frame_slice(int id, uint8_t* buffer, int buffer_size)
{
	...
	std::map<int, tlive555*>::iterator live555_it = live555s.find(id);
	VALIDATE(live555_it != live555s.end(), null_str);
	tlive555& live555 = *(live555_it->second);
	...
	if (use_chromium) {
		...
		// Create a task that will be called if we wait more than 400ms
		TaskToken task = live555.scheduler->scheduleDelayedTask(400000, TaskInterruptData, live555.video_sink);
	...
	if (live555.video_sink->received_frames % 500 == 0) {
		SDL_Log("Total received %i frames, interrupts: %i", live555.video_sink->received_frames, live555.video_sink->interrupts);
	}
