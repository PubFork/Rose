...
base_instance::base_instance(rtc::PhysicalSocketServer& ss, int argc, char** argv, int sample_rate, size_t sound_buffer_size)
	...
	, startup_servers_(false)
	, check_ip_threshold_(5000)
...
void base_instance::handle_app_event(Uint32 type)
{
	...
	if (type == SDL_APP_TERMINATING || type == SDL_QUIT) {
		SDL_Log("handle_app_event, SDL_APP_TERMINATING(0x%x)", type);
...
void base_instance::handle_resize_screen(int width, int height)
{
	VALIDATE_IN_MAIN_THREAD();
	width = posix_align_floor(width, gui2::twidget::hdpi_scale);
	height = posix_align_floor(height, gui2::twidget::hdpi_scale);

	gui2::resize_screen(width, height, true);
	preferences::set_resolution(video_, width, height);

	gui2::settings::screen_width = width;
	gui2::settings::screen_height = height;
	gui2::resize_screen(width, height, false);
}
...
bool base_instance::init_video()
{
	...
	bool nonblock_pump = false;
	int default_font_size = nposm;
	VALIDATE(game_config::longpress_time == nposm, null_str);
	tpre_setmode_settings pre_settings(gui2::twidget::current_landscape, nonblock_pump, game_config::statusbar_visible, startup_servers_, default_font_size,
		game_config::longpress_time, game_config::min_width, game_config::min_height);
	...
	if (nonblock_pump) {
		SDL_SetNonblockPump(true? SDL_TRUE: SDL_FALSE);
	}
	SDL_ShowStatusBar(game_config::statusbar_visible? SDL_TRUE: SDL_FALSE);

	if (startup_servers_) {
		// misc server require: 1)instance is valid, so must not start in base_instance::base_instaince.
		// 2)game_config::preferences_dir is valid. 3)game_config::path is valid.
		start_servers();
	}
	...
	SDL_Log("setting mode to %ix%ix32, landscape: %s", 
		resolution.first, resolution.second, gui2::twidget::current_landscape? "true": "false");
	if (!video_.setMode(resolution.first, resolution.second, video_flags)) {
		SDL_Log("required video mode, %i x %i x32 is not supported", resolution.first, resolution.second);
		return false;
	}
	SDL_Log("using mode %ix%ix32", video_.getx(), video_.gety());

	{
		const std::string default_bg_image = "assets/Default-414w-736h@3x.png";
...
bool base_instance::start_servers()
{
	VALIDATE(startup_servers_, null_str);
	...
	httpd_mgr_.start();
	live555d_mgr_.start();
	if (live555d_mgr_.url().empty()) {
		// last step is evaluate live55d_mgr_'s url.
		SDL_Log("base_instance::start_servers(), live555d_mgr_.url is empty, call stop_servers()");
		stop_servers();
		return false;
	}
	return true;
...
void base_instance::stop_servers()
{
	live555d_mgr_.stop();
	httpd_mgr_.stop();
...
bool check_ip_valid(uint32_t ip)
{
	// check this ip is valid or not.

	// Tested, the following method is not valid for win10.
	// in other words, after WLAN is disabled, ListenWithAddressAndPort still rteurn net::OK.
	// But it is valid for Xiaomi pad4. In order to improve the accuracy, this method needs to be revised in the future.
/*
...
*/
	...
	int port = 8081;
	int ret = server_socket->ListenWithAddressAndPort(addr_ss.str(), port, 1);
	SDL_Log("check_ip, ListenWithAddressAndPort(%s, %i), ret: %i", addr_ss.str().c_str(), port, ret);
...
{
	if (startup_servers_ && next_check_ip_ticks_ != 0) {
		const uint32_t now = SDL_GetTicks();
		if (ReceivingInterfaceAddr == INADDR_ANY) {
			// now think no valid ip
			if (now >= next_check_ip_ticks_) {
				SDL_Log("check_ip, %u, now is in no-ip, check...", now);
				ReceivingInterfaceAddr = get_local_ipaddr();
				if (ReceivingInterfaceAddr != INADDR_ANY) {
					start_servers();
				} else {
					next_check_ip_ticks_ = now + check_ip_threshold_;
				}
			}
		} else {
			// now think valid ip
			if (now >= next_check_ip_ticks_) {
				SDL_Log("check_ip, %u, now is in has-ip, check...", now);
				bool ok = check_ip_valid(ReceivingInterfaceAddr);
				if (!ok) {
					stop_servers();
				}
				next_check_ip_ticks_ = SDL_GetTicks() + check_ip_threshold_;
