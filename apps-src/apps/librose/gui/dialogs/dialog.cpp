...
void tdialog::base_timer_handler()
{
	...
	const int longblock_threshold = game_config::min_longblock_threshold_s * 1000;
	if (now >= last_send_longblock_ticks_ + longblock_threshold) {
		instance->will_longblock();
...
bool tdialog::show(const unsigned explicit_x, const unsigned explicit_y)
{
	if (twidget::disalbe_popup_window) {
		...
		gui2::show_message(null_str, twidget::tdisable_popup_window_lock::err_msg);
		VALIDATE(false, twidget::tdisable_popup_window_lock::err_msg);
	}
...

	{
		...
		try {
			if (timer_interval_ != 0) {
				timer_.reset(timer_interval_, *window_, boost::bind(&tdialog::base_timer_handler, this));
			}
			pre_show();
