...
public:
	...
	virtual void app_resize_screen(const int width, const int height, bool release) {}

	enum {POST_MSG_DRAG_MOUSE_LEAVE, POST_MSG_PULLREFRESH, POST_MSG_POPUP_EDITBOX, POST_MSG_MIN_APP};
...
bool run_with_progress(const std::string& message, const boost::function<bool (tprogress_&)>& did_first_drawn, bool quantify, int hidden_ms, bool show_cancel, const SDL_Rect& rect = SDL_Rect{nposm, nposm, nposm, nposm}, const boost::function<void (ttrack&, const SDL_Rect&, int, const std::string&)>& did_draw_bar = NULL);
