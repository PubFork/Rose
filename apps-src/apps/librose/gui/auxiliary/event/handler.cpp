...
class thandler: public tevent_dispatcher
{
	...
	friend void gui2::resize_screen(const int width, const int height, bool top_half);
...
void thandler::disconnect(twindow* window)
{
	...
	if (!dispatchers_.empty()) {
		twindow* topest = dispatchers_.back();
		topest->set_dirty();
		topest->clear_quiet_mouse_leaved();
		topest->window_connect_disconnected(false);
...
{
	SDL_Log("gui2::resize_screen, E, new size: %ix%i, original size: %ix%i, %s_half", 
		width, height, gui2::settings::screen_width, gui2::settings::screen_height, top_half? "top": "bottom");
	if (event::handler == nullptr) {
		return;
	}
	int at = 0;
	for (std::vector<twindow*>::const_reverse_iterator rit = event::handler->dispatchers_.rbegin(); rit != event::handler->dispatchers_.rend(); ++ rit, at ++) {
		twindow* window = *rit;
		SDL_Log("gui2::resize_screen, [%i/%i], id: %s", at, event::handler->dispatchers_.size(), window->id().c_str());
		window->resize_screen(width, height, top_half);
	}
	SDL_Log("gui2::resize_screen, X, new size: %ix%i, original size: %ix%i", width, height, gui2::settings::screen_width, gui2::settings::screen_height);
}
