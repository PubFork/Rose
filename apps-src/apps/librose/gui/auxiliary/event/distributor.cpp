...
void tdistributor::mouse_leave(const tpoint& coordinate, const tpoint& coordinate2)
{
	...
	{
		std::string err_msg = "must not popup new window during MOUSE_LEAVE. replace with app_OnMessage.";
...
/*
	SDL_Log("[#%i]signal_handler_sdl_button_down(%s)------(%i, %i), mouse_focus_: %p, mouse_click_: %p, window_id(%s)\n",
		times, distributor_.owner_.id().c_str(),
		coordinate.x, coordinate.y, distributor_.mouse_focus_, distributor_.mouse_click_,
...
{
	if (connect) {
		// new window will popup, lower of it should force to lose mouse.
		trigger_lower_quiet_mouse_leave();
	}
}

void tdistributor::lower_quiet_mouse_leave()
{
	// SDL_Log("tdistributor::lower_quiet_mouse_leave, call %s' quiet_mouse_leave", owner_.id().c_str());
	...
	static_cast<twindow*>(&owner_)->quiet_mouse_leave();
}

void tdistributor::trigger_lower_quiet_mouse_leave()
{
	std::vector<twindow*> connected = gui2::connectd_window();
	VALIDATE(connected.back() == &owner_, null_str);
	int size = connected.size();
	if (size == 1) {
		return;
	}
	twindow* dispatch = connected[size - 2];
	dispatch->distributor().lower_quiet_mouse_leave();
}
