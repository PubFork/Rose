...
{
public:
	explicit tprogress_bar(twindow& window, twidget& widget, const std::string& message, bool quantify, const boost::function<void (ttrack&, const SDL_Rect&, int, const std::string&)>& did_draw_bar);
	~tprogress_bar();
...
private:
	...
	int percentage_;
	std::string message_;
	boost::function<void (ttrack&, const SDL_Rect&, int, const std::string&)> did_draw_bar_;
