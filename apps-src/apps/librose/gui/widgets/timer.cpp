...
void ttimer::reset(const Uint32 interval, twindow& window, const boost::function<void(size_t id)>& callback)
{
...
void twindow::pump_timers() const
{
	...
	for (std::map<size_t, ttimer*>::iterator it = timers.begin(); it != timers.end(); ++ it) {
		...
		if (now < timer.next_execute_ticks) {
			continue;
		}
		if (timer.top_only && timer.window_ != this) {
			continue;
		}
		timer.execute();
		now = SDL_GetTicks();
