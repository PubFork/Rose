...
public:
	...
	void stop_drag();
	bool draging() const { return float_drag_->widget->get_visible() == twidget::VISIBLE; }
	...
	void quiet_mouse_leave() override 
	{
		if (!quiet_mouse_leaved_) {
			quiet_mouse_leaved_ = true;
			tcontainer_::quiet_mouse_leave(); 
		}
	}
	void clear_quiet_mouse_leaved() { quiet_mouse_leaved_ = false; }

	void set_did_click_edit_button(const boost::function<void (twindow&, const int)>& did)
	{
		did_click_edit_button_ = did;
	}
	void set_float_button(const std::string& label, const std::string& icon, int font_size, int color_tpl, const tpoint& size, const boost::function<void (twindow&)>& did_click, bool visible);
...
private:
	...
	struct tfloat_button {
		...
		boost::function<void (twindow&)> did_click;
	};
...
private:
	...
	bool scene_;
	bool quiet_mouse_leaved_;

	boost::function<void (twindow&, const int)> did_click_edit_button_;
