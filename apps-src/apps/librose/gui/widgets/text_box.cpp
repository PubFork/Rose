...
ttext_box::ttext_box(bool multi_line, bool password, bool atom_markup)
	: tcontrol(COUNT)
	, edit_box_param_(null_str, null_str, null_str, null_str, null_str)
...
void ttext_box::show_edit_button(const bool show)
{
	...
	if (show) {
		...
		window->set_did_click_edit_button(boost::bind(&ttext_box::did_edit_click, this, _1, _2));
...
{
	if (key != '\r' && (game_config::os == os_windows || game_config::os == os_android)) {
		return;
	}
...
void ttext_box::signal_handler_left_button_click(const event::tevent event, bool& handled)
{
	...
	if (tedit_box::instance == nullptr && edit_box_param_.valid()) {
		twindow* window = get_window();
		...
		if (y_ > window->get_height() / 3) {
			...
			edit_box_param_.def = label_;
			edit_box_param_.max_length = maximum_length_;
			tdialog::tmsg_data_popup_editbox* pdata = new tdialog::tmsg_data_popup_editbox(*this);
			rtc::Thread::Current()->Post(RTC_FROM_HERE, dialog(), tdialog::POST_MSG_POPUP_EDITBOX, pdata);
			popup_editbox = true;
		}
	}
	if (!popup_editbox) {
