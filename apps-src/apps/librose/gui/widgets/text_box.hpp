...
struct tedit_box_param 
{
	...
	tedit_box_param(const std::string& title, const std::string& prefix, const std::string& placeholder, const std::string& def, const std::string& ok, size_t max_length = -1, int cancel_strategy = show_cancel) 
		: title(title)
		, prefix(prefix)
	...
	int cancel_strategy;
	boost::function<bool (const std::string& label) > did_text_changed;
	std::string result;
};
