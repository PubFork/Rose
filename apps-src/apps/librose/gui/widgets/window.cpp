...
void twindow::signal_handler_longpress_float_button(tfloat_button& button2, bool& halt, const tpoint& coordinate)
{
	...
	start_drag(coordinate, boost::bind(&twindow::did_drag_float_button_motion, this, boost::ref(button2), _1, _2),
		boost::bind(&twindow::did_drag_float_button_leave, this, boost::ref(button2), _1, _2, _3));
...
void twindow::set_float_button(const std::string& label, const std::string& icon, int font_size, int color_tpl, const tpoint& size, const boost::function<void (twindow&)>& did_click, bool visible)
{
...
void twindow::resize_screen(const int width, const int height, bool top_half)
{
	dlg_.app_resize_screen(width, height, top_half);
	if (top_half) {
		keyboard::set_visible(false);
		gui.clear_texture();
		clear_texture();
...
void twindow::signal_handler_mouse_motion(const tpoint& coordinate)
{
	if (did_drag_mouse_motion_) {
		VALIDATE(float_drag_->widget->get_visible() == twidget::VISIBLE, null_str);
...
void twindow::signal_handler_click_dismiss(const event::tevent event, bool& handled, bool& halt)
{
	...
	// LEFT_BUTTON_CLICK can not get mouse coordiate, requrie modify it in future.
	int x, y;
