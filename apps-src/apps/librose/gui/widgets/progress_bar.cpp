...
tprogress_bar::tprogress_bar(twindow& window, twidget& widget, const std::string& message, bool quantify, const boost::function<void (ttrack&, const SDL_Rect&, int, const std::string&)>& _did_draw_bar)
	: tbase_tpl_widget(window, widget)
	, widget_(dynamic_cast<ttrack*>(&widget))
	, message_(message)
	, quantify_(quantify)
	, percentage_(0)
	, did_draw_bar_(_did_draw_bar)
	, blue_(0xff)
	, increase_(false)
	, anim_type_(nposm)
...
*/
	...
	if (did_draw_bar_ != NULL) {
		did_draw_bar_(widget, widget_rect, percentage_, message_);
		return;
	}
