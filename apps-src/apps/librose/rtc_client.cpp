...
{
	SDL_Log("trtc_client::~trtc_client(), E");
	if (state_ == CONNECTED) {
...
void trtc_client::VideoRenderer::OnFrame(const webrtc::VideoFrame& video_frame)
{
    ...
    if (now - last_ms >= 5000) {
		...
		SDL_Log("[rtc_client]#%u, %u fps, %ix%i/%ix%i, rotation(%i)", elapsed_second, frame_thread_frames_ / elapsed_second, buffer->width(), buffer->height(), video_frame.width(), video_frame.height(), video_frame.rotation());
    }
}

int trtc_client::VideoRenderer::rtsp_sessions() const
{
	const trtc_encoder* encoder_ptr = encoder_.get();
...
void trtc_client::set_renderer_texture_size(bool remote, int at, int width, int height)
{
	...
	if (vsink.requrie_encode) {
		vsink.encoder_.reset(new trtc_encoder(*this, vsink, cricket::kH264CodecName));
		VALIDATE(vsink.encoder_.get() != nullptr, null_str);
	}
	...
	SDL_Log("set_renderer_texture_size(%s)#%i is called", remote? "remote": "local", at);
}
...
void trtc_client::setup(int sessionid)
{
	...
	trtc_encoder& encoder = *vrenderer(false, 0)->encoder_.get();
...
trtspcapture::trtspcapture(int id, gui2::tdialog& dialog, tadapter& adapter, const std::vector<trtsp_settings>& rtsps, bool rtsp_exclude_local, bool idle_screen_saver)
	: trtc_client(id, dialog, adapter, tpoint(1280, nposm), false, rtsps, nposm, rtsp_exclude_local, idle_screen_saver)
	, cricket_decoder_with_params_(new CricketDecoderWithParams(std::move(nullptr)))
	, first_h264_payload_type_(200)
{
	...
	cricket_decoder_with_params_ = new CricketDecoderWithParams(std::move(std::unique_ptr<cricket::WebRtcVideoDecoderFactory>(decoder_factory)));
	decoder_factory_ = cricket::ConvertVideoDecoderFactory(std::unique_ptr<cricket::WebRtcVideoDecoderFactory>(cricket_decoder_with_params_));
...
namespace webrtc {
static VideoCodec CreateDecoderVideoCodec(const trtspcapture::Decoder& decoder) {
  VideoCodec codec;
  ...
  if (codec.codecType == kVideoCodecVP8) {
    ...
    *(codec.H264()) = VideoEncoder::GetDefaultH264Settings();
    ...
    trtspcapture::Decoder associated_decoder = decoder;
...
trtspcapture::Decoder::Decoder() = default;
trtspcapture::Decoder::Decoder(const Decoder&) = default;
trtspcapture::Decoder::~Decoder() = default;

std::string trtspcapture::Decoder::ToString() const 
{
	std::stringstream ss;
...
trtspcapture::VideoReceiveStream::VideoReceiveStream(trtspcapture& capture, std::vector<Decoder>& decoders, trtsp_settings& rtsp_settings, int at)
	: capture_(capture)
	, decoders_(decoders)
...
void trtspcapture::start_rtsp()
{
	...
	RTC_LOG(LS_INFO) << "trtspcapture::start_rtsp, recv_codecs_'s size: " << static_cast<int>(recv_codecs_.size());
	...
	for (std::vector<cricket::VideoCodec>::const_iterator it = recv_codecs_.begin(); it != recv_codecs_.end(); ++ it) {
		...
		RTC_LOG(LS_INFO) << "codec: " << codec.ToString() << ", CreateVideoDecoder success";
...
void trtspcapture::VideoReceiveStream::start()
{
	...
	for (const Decoder& decoder : decoders_) {
...
*/
	...
	rtsp_settings_.id = live555::start(rtsp_settings_.url, rtsp_settings_.httpdurl);
	...
	if (type <= 23) {
		...
		if (type == 1 || type == 5) {
			...
			if (type == 7 && rtsp_settings_.width == nposm) {
				...
				SDL_Log("rtsp's SPS, profile_idc: %u, level_idc: %u, %ux%ux%u", sps.profile_idc, sps.level_idc, sps.width, sps.height, sps.fps);
				VALIDATE(rtsp_settings_.width > 0 && rtsp_settings_.height > 0, null_str);
			}
...
trtc_encoder::trtc_encoder(trtc_client& client, trtc_client::VideoRenderer& vrenderer, const std::string& codec)
	: client_(client)
...
webrtc::VideoEncoderConfig trtc_encoder::CreateVideoEncoderConfig(const cricket::VideoCodec& codec) const
{
...
*/
  ...
  encoder_config.max_bitrate_bps = app_bitrate_kbps > 0? app_bitrate_kbps * 1000: -1;
...
void trtc_encoder::encode_frame(const rtc::scoped_refptr<webrtc::I420BufferInterface>& buffer)
{
	webrtc::VideoFrame frame(buffer, webrtc::kVideoRotation_0, 0);

	static int times = 0;
	if (times % 100 == 0) {
		SDL_Log("trtc_encoder::encode_frame, %ix%i, rotation: %i", buffer->width(), buffer->height(), frame.rotation());
	}
	times ++;
...
void trtc_encoder::setup(int sessionid)
{
...
uint32_t trtc_encoder::tsession::consume(const trtc_client& client, bool inGetAuxSDPLine, unsigned char* fTo, uint32_t fMaxSize)
{
...
uint32_t trtc_encoder::consume(int sessionid, const trtc_client& client, bool inGetAuxSDPLine, unsigned char* fTo, uint32_t fMaxSize)
{
...
void trtc_encoder::teardown(int sessionid)
{
...
void trtc_encoder::OnEncoderConfigurationChanged(std::vector<webrtc::VideoStream> streams, int min_transmit_bitrate_bps)
{
...
webrtc::VideoStreamEncoder::EncodedImageCallback::Result trtc_encoder::OnEncodedImage(
		const webrtc::EncodedImage& encoded_image,
		const webrtc::CodecSpecificInfo* codec_specific_info,
		const webrtc::RTPFragmentationHeader* fragmentation)
{
	...
	if (type == 0x7) {
		...
		SDL_Log("#%u, OnEncodedImage, receive SPS, profile_idc: %u, level_idc: %u, %ux%ux%u, _length: %i, sessions: %u", encoded_frames, sps.profile_idc, sps.level_idc, sps.width, sps.height, sps.fps, encoded_image._length, sessions_.size());

		buffered_encoded_images_.clear();
	}

	if ((int)encoded_image._length > max_image_length_) {
		max_image_length_ = encoded_image._length;
		SDL_Log("trtc_encoder::OnEncodedImage, encoded_image: %u, max_image_length: %u", encoded_image._length, max_image_length_);
	}
...
trtc_encoder::Encoder::Encoder() = default;
trtc_encoder::Encoder::Encoder(const Encoder&) = default;
trtc_encoder::Encoder::~Encoder() = default;

std::string trtc_encoder::Encoder::ToString() const 
{
	std::stringstream ss;
