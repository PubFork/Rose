...
void init(const std::string& _app, const std::string& channel)
{
	...
	// status bar
	//
	// SDL_ShowStatusBar(SDL_TRUE);
	// statusbar_visible = true; // show statusbar at startup always.

	// statusbar_height is multiplied by hdpi_scale
	statusbar_height = SDL_GetStatusBarHeight(); 
	if (os == os_windows && statusbar_height == 0) {
		// for ms-windows, statusbar_height is 0 normaly. for debug has statusbar, set a value.
		statusbar_height = 25;
	}
