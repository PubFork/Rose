...
void preprocess_res_explorer()
{
	...
	SDL_Log("create fonts directory, used %u", SDL_GetTicks() - start);
}
...
tbakreader::tbakreader(const std::string& src_name, bool use_backup, const boost::function<bool (tfile& file, int64_t dsize, bool bak)>& did_read)
...
tbakwriter::tbakwriter(const std::string& src_name, bool use_backup, const boost::function<bool (tfile& file)>& did_write)
	: src_name_(src_name)
	, use_backup_(use_backup)
