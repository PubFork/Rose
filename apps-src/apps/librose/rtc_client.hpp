...
class trtc_client: public webrtc::PeerConnectionObserver, public webrtc::CreateSessionDescriptionObserver, public sigslot::has_slots<>, public live555d::tvidcap
{
	friend class trtc_encoder;
public:
	...
	public:
		...
		bool can_safe_stop() const { return app_width_ != nposm && app_height_ != nposm; }
	...
	public:
		...
		std::unique_ptr<trtc_encoder> encoder_;
		texture tex_;
		texture cv_tex_;
		uint8_t* pixels_;
...
protected:
	void set_renderer_texture_size(bool remote, int at, int width, int height);
			...
			case POST_MSG_FIRSTFRAME:
				{
					...
					client_.set_renderer_texture_size(pdata->remote, pdata->at, pdata->width, pdata->height);
				}
				break;
			}
...
public:
	...
	trtspcapture(int id, gui2::tdialog& dialog, tadapter& adapter, const std::vector<trtsp_settings>& rtsps, bool rtsp_exclude_local, bool idle_screen_saver = true);
	~trtspcapture();
...
private:
	...
	CricketDecoderWithParams* cricket_decoder_with_params_;
	...
	{
	public:
		VideoReceiveStream(trtspcapture& capture, std::vector<Decoder>& decoders, trtsp_settings& rtsp_settings, int at);
		~VideoReceiveStream();
...
class trtc_encoder: public webrtc::VideoStreamEncoder::EncoderSink
{
	friend class webrtc::VideoStreamEncoder;
public:
	struct tsession {
		friend class trtc_client;
		tsession(trtc_encoder& encoder)
			: encoder(encoder)
			, valid_offset(0)
	...
	trtc_encoder(trtc_client& client, trtc_client::VideoRenderer& vrenderer, const std::string& codec);
	~trtc_encoder();
...
private:
	...
	typedef std::map<webrtc::SdpVideoFormat,
                     ...
                     SdpVideoFormatCompare>
        EncoderMap;
	EncoderMap allocated_encoders_;
