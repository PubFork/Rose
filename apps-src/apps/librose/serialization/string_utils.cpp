...
std::string blur_phone(const std::string& phone)
{
	VALIDATE(is_valid_phone(phone), null_str);
	// chinese mobile is 11.
	// 3 + erase_len(*) + 4. erase_len is variable, max is 4.
	const int len = phone.size();
	const int prefix_len = 3;
	const int max_erase_len = 4;
	int erase_len = 0;
	const int postfix_len = 4;
	const char* stars = "****";

	if (len <= prefix_len + postfix_len) {
		return phone;
	} else if (len < prefix_len + max_erase_len + postfix_len) {
		erase_len = len - prefix_len - postfix_len;

	} else {
		erase_len = max_erase_len;
	}

	std::string ret = phone.substr(0, prefix_len);
	ret.append(stars, erase_len);
	ret.append(phone.substr(len - postfix_len));
	return ret;
}

std::string blur_name(const std::string& name)
{
...
std::vector<trtsp_settings> parse_rtsp_string(const std::string& str, bool nocheck_0)
{
	...
	for (std::vector<std::string>::const_iterator it = vstr.begin(); it != vstr.end(); at ++) {
		...
		bool require_check = !nocheck_0 || at != 0;
		...
		if (require_check) {
			...
			if (url.find("rtsp://") != 0 || parsed_url.count(url) > 0) {
				has_error = true;
				break;
			}
		}
