...
base_manager::base_manager()
{
	...
	if (prefs.empty()) {
		SDL_Log("base_manager::base_manager, cannot find valid preferences or preferences.bak");
	}
...
std::pair<int,int> resolution(const CVideo& video)
{
	int width, height;
	if (!fullscreen()) {
	...
	if (!gui2::twidget::should_conside_orientation(width, height)) {
		// for PC, it is get screen's width/height first.
